...
//				optionItem - элемент выпадающего списка; selectedItem - выбранный элемент (блок, заменяющий select box)
...
for (i = 0; i < customSelect.length; i++){
	...
	for (j = 1; j < oldSelect.length; j++) {
		...
		optionItem.addEventListener('click', function(evt) {								//при клике на optionItem 
			...
			selectedItemLocal = this.parentNode.previousSibling;							//обратиться к выбранному элементу в текущем select
			
			for (i = 0; i < oldSelectLocal.length; i++) {									//пройтись по всем option текущего select
				if (oldSelectLocal.options[i].innerHTML == this.innerHTML) {				//находим option, текст в котором совпадает с текстом optionIem, по которому нажали
